...
def ObjectCoordinat(Camera_Orientation,Camera_Coordinate_WGS,P):
    gisement=calculate_heading(P[0],P[1],8192, 4096 , Camera_Orientation[0],Camera_Orientation[1],-Camera_Orientation[2]+90)*200/180
    ...
    return gisement,Camera_Coordinate_Lambert_72,Par_droit
####################################################################
            ...
            for n in range(len(rays[i])):
                for m in range(len(rays[j])):
                    ...
                    if intersection:
                        ...
                        # os.remove("4_Localisation/localisation/w11.txt")
                        # with open("4_Localisation/localisation/w11.txt", 'w') as file:
                        #     file.write(str(points[i][:2][0])+","+str(points[i][:2][1])+","+str(rays[i][n])+"\n")
                        #     file.write(str(points[j][:2][0])+","+str(points[j][:2][1])+","+str(rays[j][m])+"\n")
                        ...
                        print("none")
                        # os.remove("4_Localisation/localisation/w22.txt")
                        # with open("4_Localisation/localisation/w22.txt", 'w') as file:
                        #     file.write(str(points[i][:2][0])+","+str(points[i][:2][1])+","+str(rays[i][n])+"\n")
                        #     file.write(str(points[j][:2][0])+","+str(points[j][:2][1])+","+str(rays[j][m])+"\n")
